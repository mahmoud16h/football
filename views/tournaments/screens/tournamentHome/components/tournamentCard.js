...
const TournamentCard = ({ tournament, navigation, tournamentTickets, teams }) => (
  <TouchableHighlight
    key={tournament._id}
    onPress={() => navigation.navigate('Tournament', {tournament, teams})}
  ...
  >
    <View style={{display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
      ...
      {tournamentTickets.includes(tournament._id) && isToday(tournament.date.startOf('day')) &&
      <Text style={{fontSize: 18, fontWeight: 'bold', color: 'red'}}>READY</Text>}
    </View>
  </TouchableHighlight>
)
