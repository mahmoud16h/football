...
const SelectPlayers = ({ navigation, route }) => {
  ...
  const { team, isLoadingTeam, getTeam } = useTeams({ teamId: home })
  ...
  if (loadingAway || isLoadingTeam) return <LoadingScreen />
  ...
  return (
    <View keyboardShouldPersistTaps='handled' style={{ flex: 1, alignItems: 'center', marginBottom: 20 }}>
      ...
      <View style={{ display: 'flex', flexDirection: 'row' }}>
        <Card>
          <View style={{ display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
            <Text style={{ fontSize: 20, fontWeight: 'bold'}}>{capitalize(team.name)}</Text>
        ...
        <Card>
          <View style={{ display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
            <Text style={{ fontSize: 20, fontWeight: 'bold'}}>{capitalize(awayTeam.name)}</Text>
      ...
      <View style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', marginTop: 10, marginBottom: 20}}>
        {team.players.map(({ mobile, name}) => <Button width="40%" onPress={() => selectPlayer(mobile)} secondary={!selectedTeam.includes(mobile)} title={capitalize(name)}/>)}
      </View>
      <Button title="Send match request" onPress={() => console.log('sendGame Request')}/>
    </View>
  );
}
